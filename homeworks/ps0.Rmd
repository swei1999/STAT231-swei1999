...
> RESPONSE: There are 1846 rows and 3 columns. Dataset indicates which dataset the data are form, x are x-values, and y are y-values.
...
> RESPONSE: The correlation coefficient implies that there is a very weak negative linear relationship between x and y in the dino dataset.
...
> RESPONSE: There does not seem to be any visible relationship between x and y, as the output looks like a dinosaur.
...
> RESPONSE: The correlation coefficients are very similar.

```{r}
star_data <- datasaurus_dozen %>%
  filter(dataset == "star")

# Calculate a summary statistic that we call `r` equal to the correlation coefficient between `x` and `y` for the `star` dataset 
star_data %>%
  summarize(r = cor(x, y))
```
...
> RESPONSE: No, the plot looks like a star.
...
> RESPONSE: The correlation coefficient is very similar to the other two datasets.

```{r}
circle_data <- datasaurus_dozen %>%
  filter(dataset == "circle")

# Calculate a summary statistic that we call `r` equal to the correlation coefficient between `x` and `y` for the `circle` dataset 
circle_data %>%
  summarize(r = cor(x, y))
```
...
> RESPONSE: No, the plot is a circle.
...
> RESPONSE: No, because there are no relationships in any of these datasets since they are all just drawing pictures.
