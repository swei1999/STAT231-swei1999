...
> ANSWER: Genzebe Dibaba holds the indoor one-mile world record for IAAF women, and her time was 4:13.31.
...
> ANSWER: Sifan Hassan has the fastest record from an indoor or outdoor event, with a time of 4:12.33.

```{r}
women_outdoor <- html_table(tables[[8]])
women_indoor <- mutate(women_indoor, Type = "Indoor")
women_outdoor <- mutate(women_outdoor, Type = "Outdoor")
women <- bind_rows(women_indoor, women_outdoor)
head(arrange(women, Time), 1)
```
