...
> ANSWER: The reorder() function sorts drg in order of their average charge. Using x = drg, it sorts by drg from the smallest value to the largest.
...
> ANSWER: Without the theme() line, the values on the x-axis are written horizontally instead of vertically, making it impossible to see the values. Using theme() with ggplot helps customize the non-data components of the plots. 

```{r}
ggplot(data = ChargesNJ, aes(x=reorder(drg, mean_charge), y = mean_charge)) +
  geom_col(fill = "gray") +
  ylab("Statewide Average Charges ($)") + 
  xlab("Medical Procedure (DRG)")
```
...
> ANSWER: 1) What are the differences between tibble and tribble? and 2) I'm a little confused of the use of the %+% operator, what is it exactly doing in the code provided: g <- g %+% SAT_2010?
