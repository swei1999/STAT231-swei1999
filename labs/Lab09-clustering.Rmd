...
```{r}
cc <- read_csv("/Users/seanwei/Desktop/STAT231-swei1999/data/CC GENERAL.csv") 
...
cc <- cc %>% 
  ...
  select(-vars) %>%
  na.omit()
km <- kmeans(cc[,], centers = 3, nstart = 20)
km$size
```
...
> ANSWER: Cluster 1 has a lot of values in the postiives while cluster 3 has the majority of it's values in the negatives. 
...
> ANSWER: There are a lot of variables with different scales here, so the weights based on distance could have varied heavily between variables. 
...
> ANSWER: Cluster 1: Large Spenders with Large Credit, Cluster 2: Small Spenders, Cluster 3: Debt
...
> ANSWER: The biggest drop determines there should be 2 clusters, although 3 is also reasonable. One might proceed by running the test again with only 2 clusters for potentially more accurate results.
