...
> ANSWER: Grad school/PHD. Off the top of my head, you can gsub to search for patterns of - followed by text and then replace that with an empty string, but that would screw up colleges with a dash in the actual name, so a better solution is needed.
...
coordinates <- tmaptools::geocode_OSM(c(as.vector(t(gapminder["query"][, ]))),
                                 as.sf = T)

gapminder <- inner_join(gapminder, coordinates)

ggplot() +
  geom_polygon(gapminder, aes(x = lon, y = lat))
```
