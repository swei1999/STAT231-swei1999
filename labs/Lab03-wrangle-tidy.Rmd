...
```{r}
spending2 <- select(spending, Location, starts_with(c("2013", "2014")))
coverage2 <- select(coverage, Location, starts_with(c("2013", "2014")))
```

Note that there are observations both for the US as a whole and DC.  Remove these observations from both datasets.

```{r}
spending2 <- filter(spending2, Location != "United States" & 
                      Location != "District of Columbia")
coverage2 <- filter(coverage2, Location != "United States" & 
                      Location != "District of Columbia")
```
...
> RESPONSE: We need log(employer coverage) and log(spending), and each row is a year.


What are some of the steps that will need to be taken to get the data in that form?  

> RESPONSE: Transform the data, and then join these two tables.
...
```{r}
#transform spending & employer coverage
spending2 <- mutate(spending2, `2013_Log_Spending`= log(`2013__Total Health Spending`),
                               `2014_Log_Spending`= log(`2014__Total Health Spending`))
coverage2 <- mutate(coverage2, `2013_Log_Employer`= log(`2013__Employer`),
                               `2014_Log_Employer`= log(`2014__Employer`))

#join the datasets and keep only necessary features
data <- inner_join(spending2, coverage2, by = "Location")
data <- select(data, Location, `2013_Log_Spending`, `2014_Log_Spending`,
                               `2013_Log_Employer`, `2014_Log_Employer`)

#convert dataset to longer to get year & type
data_longer <- pivot_longer(data, cols = c(`2013_Log_Spending`, `2014_Log_Spending`, `2013_Log_Employer`, `2014_Log_Employer`), names_to = "year", values_to = "val")
data_longer <- mutate(data_longer, type = year)
data_longer$year <- substr(x = data_longer$year, start = 1, stop = 4)
data_longer$type <- substr(x = data_longer$type, start = 6, stop = 17)

#convert dataset back to wider
data_wider <- pivot_wider(data_longer, names_from = type, values_from = val)
data_wider <- clean_names(data_wider)

#create scatterplot
ggplot(data_wider, aes(x = log_spending, y = log_employer)) + 
  geom_point(aes(color = year)) + 
  labs(x = "Log Spending", y = "Log Employer") + 
  ggtitle("Log (Employer Coverage) vs Log (Spending)")
