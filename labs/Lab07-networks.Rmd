...
> ANSWER: Both have it's uses.

Might the color scheme in the plot below be more effective?

> ANSWER: Yes, it could've been harder to distinguish between the blues in the previous one.
...
> ANSWER: Different countries can stand out (i.e Russia)
...
> ANSWER: 107 characters

```{r}
# create as network object
got_network <- graph_from_data_frame(got, directed = FALSE)
...
> ANSWER: 352 edges, density = 0.06207018

```{r}
ecount(got_network)
# note that the number of *possible* edges in an undirected graph is (V choose 2) = V*(V-1)/2
choose(vcount(got_network), 2)
# density
ecount(got_network)/choose(vcount(got_network), 2)
```
...
> ANSWER: TRUE, the graph is connected. There is an edge from one node to every other node in the graph. 
...
> ANSWER: 6 - the least number of conversations between two characters was 6?
...
```{r}
V(got_network)$degree <- igraph::degree(got_network)
V(got_network)$wtdegree <- strength(got_network
                               , weights=E(got_network)$Weight)
stats <- data.frame(name = V(got_network)$name,
                   degree = V(got_network)$degree,
                   wtdegree = V(got_network)$wtdegree )
stats
```
...
> ANSWER: Tyrion, Jon, Sansa, Robb, Jaime
...
> ANSWER: Rob interacts with more different people but Bran had more interactions with his connectoins.
...
> ANSWER: Top betweeness is Jon, Robert, Tyrion, Daenerys, and Robb

```{r}
V(got_network)$betweenness <- igraph::betweenness(got_network, weights=NA)
stats$betweenness <- V(got_network)$betweenness
stats %>% 
...
> ANSWER: Tyrion, Jon, Rob, Sansa, Daenerys

```{r}
stats <- stats %>%
 mutate(eigen = eigen_centrality(got_network, directed = FALSE
                         , weights = NA)$vector
        , page.rank = page_rank(got_network, directed = FALSE
                         , weights = NA)$vector
 )
stats %>% 
...
```{r}
ggplot(data =  ggnetwork(got_network)
       , aes(x = x, y = y, xend = xend, yend = yend)) +
  geom_edges(arrow=arrow(type="closed", length=unit(6,"pt"))
            , color = "lightgray") +
  geom_nodes() +
